...
-- One row per completed Stripe checkout for the Strive Dribbling System.
-- Written by /api/ghl/dribbling-course-buyer (fired from the success page)
-- so even without a Stripe webhook, every sale lands in Supabase.
...
create table if not exists course_sales (
  ...
  product                   text not null default 'Strive Dribbling System',
...
-- Internal OS — open RLS. Tighten when auth ships.
