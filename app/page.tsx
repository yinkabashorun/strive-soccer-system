...
async function loadDashboard(): Promise<DashboardData> {
  ...
  try {
    ...
    const todaySessions =
      (sessionsRes.data as Session[] | null) ?? [];
    return {
  ...
  return (
    <div>
      <PageHeader
        ...
        subtitle="Active clients, today's schedule, fresh leads, and one-tap content. Built so you spend the day coaching, not clicking."
        actions={
          <>
      ...
      {/* One-click content takes the prime slot */}
      <div className="mt-6">
        <OneClickContent />
      </div>
        ...
        <section className="card p-5">
          ...
          {data.topWon.length === 0 ? (
            <div className="rounded-xl border border-dashed border-white/10 p-6 text-center text-xs text-muted">
              No Won contacts yet. Run the GHL bulk sync from{" "}
      ...
      <div className="mt-6 flex flex-wrap items-center gap-2 text-[10px] uppercase tracking-[0.18em] text-muted">
        ...
        Stats above are live from Supabase · sync GHL from{" "}
        <Link href="/integrations" className="text-accent hover:underline">
          Integrations
        </Link>
      </div>
